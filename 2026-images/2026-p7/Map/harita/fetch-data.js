...
async function main() {
  ...
  const onlyCategory = args.find(a => Object.keys(CATEGORY_CLAUSES).includes(a));
  const onlyYears = args.map(Number).filter(Boolean);
  const years = onlyYears.length ? onlyYears : Array.from({ length: 16 }, (_, i) => 2010 + i);
  const categories = onlyCategory ? [onlyCategory] : Object.keys(CATEGORY_CLAUSES);
  const delayMs = 5000;
